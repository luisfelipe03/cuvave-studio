...
function clampValues(values: PresetValues): PresetValues {
...
function buildSystemPrompt(guitar?: string): string {
...
export const generatePreset = onCall(
  { secrets: ['DEEPSEEK_API_KEY'], timeoutSeconds: 120, memory: '256MiB' },
  async (req) => {
    const uid = req.auth?.uid
    if (!uid) {
      throw new HttpsError('unauthenticated', 'Entre com o Google primeiro.')
    }
    ...
    if (uid !== ownerUid) {
      ...
      if (!accessSnap.exists || accessSnap.data()?.allowed !== true) {
        throw new HttpsError(
          'permission-denied',
          'Sua conta não tem acesso à chave compartilhada. Peça acesso ou configure a sua própria chave.',
        )
      }
    }
    ...
    if (!apiKey) {
      throw new HttpsError(
        'failed-precondition',
        'A chave compartilhada não foi configurada no servidor ainda.',
      )
    }

    const { song, hint, guitar } = req.data as {
      song?: string
      hint?: string
      guitar?: string
    }
    if (!song || !song.trim()) {
      throw new HttpsError('invalid-argument', 'Nome da música é obrigatório.')
    }
    ...
    const res = await fetch(CHAT_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Monte um preset para tocar "${song.trim()}".${
    ...
    if (!res.ok) {
      if (res.status === 401)
        throw new HttpsError(
          'failed-precondition',
          'A chave compartilhada foi recusada pela DeepSeek — o dono precisa verificar o segredo.',
        )
      if (res.status === 402)
        throw new HttpsError(
          'failed-precondition',
          'A conta DeepSeek do dono está sem saldo.',
        )
      if (res.status === 429)
        throw new HttpsError(
          'resource-exhausted',
          'Muitas gerações ao mesmo tempo. Espere alguns segundos.',
        )
      throw new HttpsError(
        'internal',
        `A DeepSeek recusou a requisição (HTTP ${res.status}).`,
      )
    }
    ...
    if (!content) {
      throw new HttpsError('internal', 'A DeepSeek respondeu vazio.')
    }
    ...
    if (!parsed.preset || typeof parsed.preset !== 'object') {
      throw new HttpsError('internal', 'A resposta da IA veio sem o preset.')
    }
